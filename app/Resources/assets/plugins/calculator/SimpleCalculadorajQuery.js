 ...
 */
 jQuery.fn.extend({Calculadora: function(op) {
					...
					defaults = {
						TituloHTML:' <div class="panel-heading"><b>Calculator</b> <a id="calculator-close"  class="pull-right btn red mini"><i class="icon-remove icon-white"></i></a></div>',
						Botones:["7","8","9","+","4","5","6","-","3","2","1","*","0",".","=","/"],
						Signos:["+", "-", "*", "/"],
						ClaseBtns1: 'primary',
						ClaseBtns2: 'success',
						ClaseBtns3: 'warning',
						ClaseColumnas:'span3 mbottom',
						ClaseBotones:'btn3d btn-lg btn-block btn btn-',
						txtSalida:idInstancia+'txtResultado',
						ClasetxtSalida:'form-control txtr span12 m-wrap',
					...
					$(LaCalculadora).append('<div class="row-fluid" id="'+idInstancia+'btns"></div>');					$.each(Botones, function(index,value) {
						Clase=op.ClaseBtns1			
					...
					$(LaCalculadora).html('<div class="panel panel-calculator btn-block calculadoraBase  mtop">'+op.TituloHTML+'<div class="panel-body"><div class="row-fluid"><div class="span12" style="margin-bottom: 10px;">'+$(LaCalculadora).html()+'</div></div></div> </div>');
