...
use Setting\Bundle\ContentBundle\Form\MallShopType;
...
class MallShopController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $entities = $em->getRepository('SettingContentBundle:MallConnect')->findBy(array('mall'=>$globalOption));
        return $this->render('SettingContentBundle:MallConnect:mall.html.twig', array(
            'entities' => $entities,
        ));
    ...
    public function createAction(Request $request)
    {
        ...
        if ($form->isValid() and $this->checkDuplicateEntry($entity) == true) {
                ...
                $em = $this->getDoctrine()->getManager();
                $mall = $this->getUser()->getGlobalOption();
                $entity->setMall($mall);
                $entity->upload();
                $em->persist($entity);
                $em->flush();
                ...
                return $this->redirect($this->generateUrl('mallshop'));
        }
        $this->get('session')->getFlashBag()->add(
            'error',"This data is already in use"
        );
        return $this->render('SettingContentBundle:MallConnect:edit.html.twig', array(
            'entity' => $entity,
    ...
    {
        $form = $this->createForm(new MallShopType(), $entity, array(
            'action' => $this->generateUrl('mallshop_create'),
            'method' => 'POST',
            'attr' => array(
    ...
    public function showAction($id)
    {
        ...
        $entity = $em->getRepository('SettinMallShopTypegContentBundle:MallConnect')->find($id);
    ...
    */
    ...
    {
        $form = $this->createForm(new MallShopType(), $entity, array(
            'action' => $this->generateUrl('mallshop_update', array('id' => $entity->getId())),
            'method' => 'PUT',
            'attr' => array(
                'class' => 'horizontal-form',
                'novalidate' => 'novalidate',
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        if ($editForm->isValid()) {
            ...
            return $this->redirect($this->generateUrl('mallshop_edit', array('id' => $id)));
        }
    ...
    public function statusAction(Request $request, $id)
    {
        ...
        $status = $entity->getMallStatus();
        if($status == 1){
            $entity->setMallStatus(0);
        } else{
            $entity->setMallStatus(1);
        }
        $em->flush();
        $this->get('session')->getFlashBag()->add(
