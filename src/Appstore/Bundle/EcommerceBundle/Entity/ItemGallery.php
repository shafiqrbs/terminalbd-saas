 ...
 * ProductGallery
 *
 * @ORM\Table("ecommerce_item_gallery")
 * @ORM\Entity(repositoryClass="Appstore\Bundle\EcommerceBundle\Repository\ItemGalleryRepository")
 */
class ItemGallery
{
    /**
    ...
    /**
     * @ORM\ManyToOne(targetEntity="Appstore\Bundle\EcommerceBundle\Entity\Item", inversedBy="itemGalleries" )
    ...
    public function getWebPath($globalOption,$item)
    {
        return null === $this->path
            ? null
            : $this->getUploadDir().'/'.$globalOption.'/ecommerce/item/'.$item.'/gallery/'.$this->path;
    }

    protected function getUploadRootDir($globalOption,$item)
    {
      //return 'uploads/files/'.$user.'/inventory/item/'.$item.'/gallery/';
      return WEB_PATH.'/'.$this->getUploadDir().'/'.$globalOption.'/ecommerce/item/'.$item.'/gallery/';
    }
