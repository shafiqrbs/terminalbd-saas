...
class StockIssueListener
{
    ...
    public function createCode(LifecycleEventArgs $args)
    {
        ...
        if ($entity instanceof StockIssue) {

            $datetime = new \DateTime("now");
            $lastCode = $this->getLastCode($args, $datetime, $entity);
            $entity->setCode($lastCode+1);
            $entity->setInvoice(sprintf("%s%s%s","ISU/N-",$datetime->format('my'), str_pad($entity->getCode(),4, '0', STR_PAD_LEFT)));
     ...
     * @return int|mixed
     */
    public function getLastCode(LifecycleEventArgs $args, $datetime, StockIssue $entity)
    {

        $start = $datetime->format('Y-m-01 00:00:00');
        $end = $datetime->format('Y-m-t 23:59:59');
        $entityManager = $args->getEntityManager();
        $qb = $entityManager->getRepository('AssetsBundle:StockIssue')->createQueryBuilder('s');
        $qb
            ->select('MAX(s.code)')
