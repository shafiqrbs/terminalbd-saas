    ...
    protected function getNamedArray(\PHPExcel $objPHPExcel)
    {
        ...
        for ($row = 2; $row <= $highestRow; ++$row) {
            ...
            if ((isset($dataRow[$row]['A'])) && ($dataRow[$row]['A'] > '')) {
                ...
                $key = $dataArray['Caliber'].$dataArray['Name']. $dataArray['ProductGroup']. $dataArray['Category'].$dataArray['Model'].$dataArray['Brand'].$dataArray['Country'].$dataArray['Description'].$dataArray['Price'].$dataArray['Stock'];
